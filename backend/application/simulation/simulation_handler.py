...
class SimulationHandler:
    def __init__(self, service: StadisticService) -> None: #INYD
    ...
    async def handle_set_seed(self, sio, sid, data: dict) -> None:
        ...
        if dias is None:
            await sio.emit( #EL BACKEND EMITE UN MJE DE ERROR
                "simulationDashboard",
...
#NONE - SIMI
        ...
        for dia in range(1, dias + 1):
            self.service.simular_lote(dia) # self.service llama a su servicio que se le inyecto
            await sio.emit(
                "simulationDashboard",
                self.service.crear_dashboard(
                    dias_totales=dias,
